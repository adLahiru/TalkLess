...
// ------------------------------------------------------------
// Custom decoder backends for OGG/Vorbis support
// ------------------------------------------------------------
static ma_decoding_backend_vtable* g_pCustomBackends[] = {&g_ma_decoding_backend_vtable_stbvorbis};
static const ma_uint32 g_customBackendCount = sizeof(g_pCustomBackends) / sizeof(g_pCustomBackends[0]);

// Helper to configure decoder with OGG support
static ma_decoder_config configureDecoderWithVorbis(ma_format format, ma_uint32 channels, ma_uint32 sampleRate)
{
    ma_decoder_config cfg = ma_decoder_config_init(format, channels, sampleRate);
    cfg.ppCustomBackendVTables = g_pCustomBackends;
    cfg.customBackendCount = g_customBackendCount;
    return cfg;
}
...
void AudioEngine::decoderThreadFunc(AudioEngine* engine, ClipSlot* slot, int slotId, uint64_t token)
{
    ...
    ma_decoder_config cfg = configureDecoderWithVorbis(ma_format_f32, 2, engine->m_sampleRate);
    ma_decoder dec;
...
std::pair<double, double> AudioEngine::loadClip(int slotId, const std::string& filepath)
{
    ...
    // duration
    double endSec = -1.0;
    ma_decoder_config cfg = configureDecoderWithVorbis(ma_format_f32, 2, m_sampleRate);
    ma_decoder dec;
    if (ma_decoder_init_file(filepath.c_str(), &cfg, &dec) == MA_SUCCESS) {
