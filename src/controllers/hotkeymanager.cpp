...
HotkeyManager::HotkeyManager(AudioEngine* audioEngine, QObject* parent)
    : QObject(parent), m_audioEngine(audioEngine), m_globalHotkeysEnabled(true), m_hotkeyPollTimer(nullptr)
#ifdef Q_OS_MAC
      ,
      m_nextHotkeyId(1)
...
void HotkeyManager::saveHotkeys()
{
    ...
    settings.endGroup();
    settings.sync();
    qDebug() << "Saved" << m_clipHotkeys.size()
             << "hotkeys to settings (globalHotkeysEnabled:" << m_globalHotkeysEnabled << ")";
}
...
bool HotkeyManager::registerSystemHotkey(const QString& clipId, const QString& keySequence, quint32 hotkeyId)
{
    ...
    if (status == noErr) {
        ...
        qDebug() << "Registered system hotkey:" << keySequence << "for" << (hotkeyId >= 1000 ? "system action" : "clip")
                 << ":" << clipId << "with ID:" << hotkeyId;
        return true;
    }
