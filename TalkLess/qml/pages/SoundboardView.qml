                ...
                Flickable {
                    ...
                    ColumnLayout {
                        ...
                        ColumnLayout {
                            ...
                            // ===== PLAYBACK POSITION / SEEK SLIDER =====
                            ColumnLayout {
                                id: seekSection
                                Layout.fillWidth: true
                                spacing: 4

                                // Store pending seek position (normalized 0-1) for "play from here"
                                property real pendingSeekPosition: -1
                                property bool hasPendingSeek: pendingSeekPosition >= 0

                                // Waveform data for visualization (array of 0-1 amplitudes)
                                property var waveformData: []
                                property var mockWaveformData: generateMockWaveform()
                                ...
                                function generateMockWaveform() {
                                    ...
                                    for (var i = 0; i < 30; i++) {
                                        ...
                                        data.push(Math.max(0.1, Math.min(1.0, base + noise)));
                                    }
                                    return data;
                                }

                                // Get the waveform to display (real or mock)
                                property var displayWaveform: waveformData.length > 0 ? waveformData : mockWaveformData

                                // Load waveform when clip is selected - using new simplified method
                                function loadWaveformForClip(clipId) {
                                    if (soundboardService)
                                        soundboardService.logFromQml("=== LOADING WAVEFORM for " + clipId);
                                    ...
                                    if (peaks && peaks.length > 0) {
                                        ...
                                        if (soundboardService)
                                            soundboardService.logFromQml("No peaks returned, using mock");
                                        waveformData = [];
                                    }
                                }

                                // Clear pending seek when clip changes or starts playing
                                Connections {
                                    target: root
                                    function onSelectedClipIdChanged() {
                                        seekSection.pendingSeekPosition = -1;
                                        seekSection.loadWaveformForClip(root.selectedClipId);
                                    }
                                }

                                // Load waveform on initial component load
                                Component.onCompleted: {
                                    if (soundboardService)
                                        soundboardService.logFromQml("=== SOUNDBOARD VIEW LOADED (v3) ===");
                                    loadWaveformForClip(root.selectedClipId);
                                }
                                ...
                                RowLayout {
                                    ...
                                    Text {
                                        ...
                                        property real currentProgress: {
                                            // If we have a pending seek and clip is not playing, show pending position
                                            if (seekSection.hasPendingSeek) {
                                                var playingIds = soundboardService?.playingClipIDs() ?? [];
                                                var isPlaying = playingIds.indexOf(root.selectedClipId) !== -1;
                                                if (!isPlaying) {
                                                    return seekSection.pendingSeekPosition;
                                                }
                                            }
                                            if (root.selectedClipId === -1)
                                                return 0;
                                            var prog = progressUpdateTimer.clipProgressMap[root.selectedClipId];
                                            return prog !== undefined ? prog : 0;
                                        }
                                        property real durationMs: clipEditorTab.durationSec * 1000
                                        property real currentMs: currentProgress * durationMs
                                        ...
                                        color: seekSection.hasPendingSeek ? Colors.accent : Colors.textSecondary
                                        font.family: interFont.status === FontLoader.Ready ? interFont.name : "Arial"
                                        font.pixelSize: 11
                                    }
                                }
                                ...
                                Item {
                                    ...
                                    // Waveform track with progress overlay
                                    Item {
                                        id: seekTrack
                                        anchors.left: parent.left
                                        anchors.right: parent.right
                                        ...
                                        Row {
                                            ...
                                            Repeater {
                                                model: seekSection.displayWaveform ? seekSection.displayWaveform.length : 0

                                                Rectangle {
                                                    id: waveformBar
                                                    required property int index

                                                    property int barCount: seekSection.displayWaveform ? seekSection.displayWaveform.length : 0
                                                    width: barCount > 0 ? (waveformBars.width - (waveformBars.spacing * (barCount - 1))) / barCount : 4
                                                    height: parent.height
                                                    color: "transparent"

                                                    // robust access using index
                                                    property real amplitude: (seekSection.displayWaveform && seekSection.displayWaveform[index] !== undefined) ? seekSection.displayWaveform[index] : 0.3
                                                    property real normalizedPosition: barCount > 0 ? index / barCount : 0
                                                    property bool isPlayed: normalizedPosition < seekSliderItem.displayProgress

                                                    Rectangle {
                                                        anchors.centerIn: parent
                                                        width: parent.width
                                                        height: Math.max(4, parent.height * waveformBar.amplitude * 0.9)
                                                        radius: width / 2
                                        ...
                                        // Semi-transparent overlay for the "unplayed" portion (optional visual enhancement)
                                        Rectangle {
                                            anchors.left: parent.left
                                            anchors.top: parent.top
                                            anchors.bottom: parent.bottom
                                            width: parent.width * seekSliderItem.displayProgress
                                            color: "transparent"
                                        }
                                    }
                                    ...
                                    Rectangle {
                                        ...
                                        height: 14
                                        radius: 7
                                        color: seekHandleArea.containsMouse || seekSliderItem.isDragging || seekSection.hasPendingSeek ? Colors.accent : Colors.textOnPrimary
                                        x: seekTrack.x + (seekTrack.width - width) * seekSliderItem.displayProgress
                                        ...
                                        // Pulsing animation when pending seek exists
                                        SequentialAnimation on scale {
                                            running: seekSection.hasPendingSeek
                                            loops: Animation.Infinite
                                            NumberAnimation {
                                                to: 1.15
                                                duration: 500
                                    ...
                                    MouseArea {
                                        ...
                                        function updateSeekPosition(mouseX) {
                                            var normalized = Math.max(0, Math.min(1, mouseX / seekTrack.width));
                                            parent.dragValue = normalized;
                                        }
                                        ...
                                        function applySeek() {
                                            ...
                                            var durationMs = clipEditorTab.durationSec * 1000;
                                            var positionMs = parent.dragValue * durationMs;

                                            // Account for trim start if applicable
                                            var trimStartMs = clipEditorTab.trimStartMs || 0;
                                            positionMs += trimStartMs;

                                            if (isPlaying) {
                                                // Clip is playing, seek immediately
                                                soundboardService.seekClip(activeClipsModel.boardId, root.selectedClipId, positionMs);
                                                seekSection.pendingSeekPosition = -1;
                                            } else {
                                                // Clip is not playing, store the seek position for later
                                                seekSection.pendingSeekPosition = parent.dragValue;
                                            }
                                        }
                                    }
                                }

                                // Play from position button and hint
                                RowLayout {
                                    Layout.fillWidth: true
                                    spacing: 8

                                    // Play from position button - visible when there's a pending seek
                                    Rectangle {
                                        Layout.preferredWidth: seekSection.hasPendingSeek ? 110 : 0
                                        Layout.preferredHeight: 28
                                        visible: seekSection.hasPendingSeek
                                        radius: 14
                                        gradient: Gradient {
                                            orientation: Gradient.Horizontal
                                            GradientStop {
                                                position: 0.0
                                                color: playFromPosArea.containsMouse ? Colors.accentLight : Colors.accent
                                            }
                                            GradientStop {
                                                position: 1.0
                                                color: playFromPosArea.containsMouse ? Colors.accent : Colors.accentMedium
                                            }
                                        }
                                        ...
                                        RowLayout {
                                            ...
                                            Image {
                                                id: playFromPosIcon
                                                source: "qrc:/qt/qml/TalkLess/resources/icons/play.svg"
                                                width: 12
                                                height: 12
                                                visible: false
                                            }
                                            ...
                                            MultiEffect {
                                                source: playFromPosIcon
                                                width: 12
                                                height: 12
                                                colorization: 1.0
                                                colorizationColor: Colors.textOnPrimary
                                            }
                                        ...
                                        MouseArea {
                                            ...
                                            onClicked: {
                                                ...
                                                var durationMs = clipEditorTab.durationSec * 1000;
                                                var positionMs = seekSection.pendingSeekPosition * durationMs;

                                                // Account for trim start
                                                var trimStartMs = clipEditorTab.trimStartMs || 0;
                                                positionMs += trimStartMs;

                                                console.log("=== PLAY FROM POSITION DEBUG ===");
                                                console.log("pendingSeekPosition:", seekSection.pendingSeekPosition);
                                                console.log("durationSec:", clipEditorTab.durationSec);
                                                console.log("durationMs:", durationMs);
                                                console.log("trimStartMs:", trimStartMs);
                                                console.log("Final positionMs:", positionMs);
                                                console.log("clipId:", root.selectedClipId);

                                                // Use the new method that plays and seeks atomically
                                                soundboardService.playClipFromPosition(root.selectedClipId, positionMs);
                                    ...
                                    // Hint text
                                    Text {
                                        text: seekSection.hasPendingSeek ? "Position set - click Play to start" : "Click or drag to seek"
                                        color: seekSection.hasPendingSeek ? Colors.accent : Colors.textDisabled
                                        font.family: interFont.status === FontLoader.Ready ? interFont.name : "Arial"
                                        font.pixelSize: 9
                                        visible: root.selectedClipId !== -1
                                    }
